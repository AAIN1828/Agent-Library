"""Internal helpers for driving the Response Delivery agent chain."""
    ...
    for _ in range(MAX_CHAIN_ROUNDS):
        ...
        for key, value in parsed.items():
            if value is not None:
                state[key] = value
        if content and "delivered_answer" not in state and not parsed.get("delivered_answer"):
            state.setdefault("raw_content", content.strip())
